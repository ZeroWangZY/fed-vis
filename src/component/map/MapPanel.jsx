...
class MapPanel extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.deleteRect !== -1 &&
      this.props.deleteRect !== prevProps.deleteRect
    ) {
      //删除指定rect
      if (
        document.getElementsByClassName("drawRect-" + this.props.deleteRect)
          .length > 0
      ) {
        // 如果直接删的矩形 则元素已不存在
        let rect = document.getElementsByClassName(
          "drawRect-" + this.props.deleteRect
        )[0];
      ...
      this.props.selectRect !== -1 &&
      this.props.selectRect !== prevProps.selectRect
    ) {
      // 创建新的rect 把之前的rect隐藏
      let prevRects = document.getElementsByClassName("drawRect");
      ...
      document.getElementsByClassName(
        "drawRect-" + this.props.selectRect
