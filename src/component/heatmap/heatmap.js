...
export default withLeaflet(class HeatmapLayer extends MapLayer {
  ...
  fitBounds() {
    ...
    if (shouldIgnoreLocation(ne) || shouldIgnoreLocation(sw) || !isFinite(ne) || !isFinite(sw)) {
        return;
    }
  ...
  componentDidUpdate(prevProps) {
    ...
    // const currentProps = this.props;
    // const nextHeatmapProps = this.getHeatmapProps(prevProps);

    // if (currentProps.gradient!==nextHeatmapProps.gradient)
    //     this.updateHeatmapGradient(nextHeatmapProps.gradient);
  ...
  redraw() {
    ...
    const roundResults = (results) => results.reduce((result, row) =>
        filterUndefined(row).map((cell) => [
            Math.round(cell[0]),
            Math.round(cell[1]),
    ...
    const accumulateInGrid = (points, leafletMap, bounds) => points.reduce((grid, point) => {
