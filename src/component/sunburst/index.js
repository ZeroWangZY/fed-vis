...
class Sunburst extends React.Component {
  constructor(props) {
    ...
    this.radius = Math.min(this.chartSize[0], this.chartSize[1]) / 2;
  }
  ...
  render() {
    ...
    const colorMap = this.genColorMap(data);
    ...
    const pathData = root.descendants().filter(d => d.depth);
    const textData = root.descendants().filter(d => d.depth && (d.y0 + d.y1) / 2 * (d.x1 - d.x0) > 10);

    return (
      <svg className="sunburst" style={svgStyles}>
        <g style={groupStyles}>
        {
          pathData.map((d, i) => (
            <path key={i}
        ...
        }
        {
          textData.map((d, i) => (
            <text key={i}
              transform={this.genTextTranslate(d)}
              dy="0.35em"
            >
              {d.data.name}
            </text>
