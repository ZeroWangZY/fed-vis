...
def get_origin_polar(data):
    data = data['data'][:1]
    data[0]['round'] = 0
    data[0]['server'] = {
        "diagram_data": [
            np.array(data[0]['server']['ground_true']).reshape(7, 24).tolist(),
            np.zeros((7, 24)).tolist()
        ],
        "re":
        0,
        "loss": []
...
def get_normal_polar(data):
    data = get_origin_polar(data)
    clients = data['data'][0]['clients']
    server = data['data'][0]['server']
